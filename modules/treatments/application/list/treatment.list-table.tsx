...
interface TreatmentListTableProps {
  ...
  setActiveTreatment: Dispatch<SetStateAction<Treatment>>
}

const TreatmentListTable = ({ handleModal, treatments, totalPages, handlePageChange, setActiveTreatment }: TreatmentListTableProps) => {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="flex flex-col">
        ...
        {typeof(treatments) !== "undefined" && treatments.map((treatment, key) => (
          <div
            className={`grid grid-cols-3 sm:grid-cols-4 ${
              key === treatments.length - 1
                ? ""
          ...
          >
            <div className="flex items-center justify p-2.5 xl:p-5">
              <p className="text-black dark:text-white">{treatment.name}</p>
            </div>

            <div className="flex items-center justify-center p-2.5 xl:p-5">
              <p className="text-black dark:text-white">{treatment.code}</p>
            </div>

            <div className="hidden items-center justify-center sm:flex p-2.5 xl:p-5">
              <p className="text-meta-3"><Currency value={treatment.price} /></p>
            </div>
            ...
            <div className="items-center justify-center p-2.5 sm:flex xl:p-5">
              <ul className="flex items-center gap-2 2xsm:gap-4">
                <motion.li className="relative" whileHover={{ scale: 1.2, transition: { duration: 0.2 }}} whileTap={{ scale:0.9 }} >  
                  <Link
                    href="#"
                    onClick={() => { handleModal(false, true); setActiveTreatment(treatment) }}
                ...
                <motion.li className="relative" whileHover={{ scale: 1.2, transition: { duration: 0.2 }}} whileTap={{ scale:0.9 }} >  
                  <Link
                    href="#"
                    onClick={() => { handleModal(false, false); setActiveTreatment(treatment) }}
