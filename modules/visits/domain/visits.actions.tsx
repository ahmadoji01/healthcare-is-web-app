...
export const getTotalVisitsWithFilter = (token:string, filter:object) => 
	directusClient.request( 
		withToken(token, aggregate('visits', { aggregate: { count: '*' }, query: { filter } })) 
	)
...
export const getVisitByCount = (token:string, filter:object, groupBy:string) => 
	directusClient.request( withToken(token, aggregate('visits', { aggregate: { count: '*' }, groupBy: [groupBy], query: { filter } }  )) )